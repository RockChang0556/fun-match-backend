...
import { IWechatOAuthFailure, IWechatOAuthSuccess } from './wechat-auth.interface';
...
export class WechatAuthService {
  ...
  async getWechatOAuth(code: string): Promise<IWechatOAuthSuccess | IWechatOAuthFailure> {
    const appId = process.env.WECHAT_APP_ID;
    ...
    const res = (await lastValueFrom(observe)) as any;
    return res?.data;
  }
}
